...
Clock::Clock(DisplayApp* app,
  ...
  lv_label_set_text(heartbeatValue, "[L_HR]#ee3311 --- bpm#");
...
bool Clock::Refresh() {
 ...
 if ((year != currentYear) || (month != currentMonth) || (dayOfWeek != currentDayOfWeek) || (day != currentDay)) {
  ...
  if(heartbeat.IsUpdated() || heartbeatRunning.IsUpdated()) {
    char heartbeatBuffer[34];
    if(heartbeatRunning.Get())
      sprintf(heartbeatBuffer, "[L_HR]#ee3311 %d bpm#", heartbeat.Get());
    else
      sprintf(heartbeatBuffer, "[L_HR]#ee3311 --- bpm#");
  ...
  if(stepCount.IsUpdated()) {
    char stepBuffer[36];
    sprintf(stepBuffer, "[STEP]#ee3377 %lu steps#", stepCount.Get());
